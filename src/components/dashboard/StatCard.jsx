...
export const StatCard = ({
  ...
  >
    <div
      className={`absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-20 transition-opacity duration-500 transform group-hover:scale-110 ${color}`}
    >
      <Icon size={100} />
    </div>
    ...
    <div
      className={`absolute inset-0 bg-gradient-to-br ${color.replace("text-", "from-")}/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500`}
    />
  </motion.div>
);
